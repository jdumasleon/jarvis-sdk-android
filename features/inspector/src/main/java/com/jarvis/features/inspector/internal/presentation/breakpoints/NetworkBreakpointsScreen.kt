...
import com.jarvis.core.internal.designsystem.component.*
 ...
 */
@Composable
fun NetworkBreakpointsScreen(
    actionKey: String,
    modifier: Modifier = Modifier,
    viewModel: NetworkBreakpointsViewModel = hiltViewModel()
) {
...
@Composable
private fun NetworkBreakpointsScreen(
    uiState: NetworkBreackpointsUiState,
...
@Composable
private fun NetworkBreakpointsContent(
    data: NetworkBreakpointsData,
    onEvent: (NetworkRulesEvent) -> Unit,
    modifier: Modifier = Modifier
) {
    ...
    Box(
        ...
        LazyColumn(
            ...
            if (data.applicationHistory.isNotEmpty()) {
                ...
                items(
                    items = data.applicationHistory.take(10),
                    key = { application -> "${application.ruleId}_${application.timestamp}" }
                ) { application ->
...
@Composable
private fun NetworkBrackpointsHeader(
    rulesCount: Int,
    ...
    DSJarvisTheme {
        NetworkBreakpointsScreen(
            uiState = ResourceState.Success(
                NetworkBreakpointsData.mockBreakpointsData()
            ),
            onEvent = {}
        )
    }
