...
data class ItemInterval(val start: Float = 0f, val size: Int = 0) {
    val center: Float
        get() = start + size / 2
    val end: Float
        get() = start + size
...
class DSReorderableListState internal constructor(
    listSize: Int,
    spacing: Float = 0f,
...
internal interface DSReorderableListItemScope {
    /**
...
internal class DSReorderableListItemScopeImpl(
    private val state: DSReorderableListState,
    private val index: Int,
) : DSReorderableListItemScope {
...
open class DSReorderableListScope(
    private val state: DSReorderableListState,
     ...
     * @param content The content of the item. The content can use the [DSReorderableListItemScope.draggableHandle] modifier to make the item draggable.
     */
    @Composable
    internal fun DSReorderableItem(
        modifier: Modifier = Modifier,
        content: @Composable DSReorderableListItemScope.() -> Unit,
    ) {
        Box(
            modifier = modifier
            ...
            DSReorderableListItemScopeImpl(
                state = this@DSReorderableListScope.state,
                index = this@DSReorderableListScope.index
            ).content()
        }
    }
}

class DSReorderableColumnScope(
    state: DSReorderableListState,
    index: Int,
    isAnimating: Boolean,
    private val scope: ColumnScope
) : DSReorderableListScope(state, orientation = Orientation.Vertical, index, isAnimating),
    ColumnScope by scope

class DSReorderableRowScope(
    state: DSReorderableListState,
    index: Int,
    isAnimating: Boolean,
    private val scope: RowScope
) : DSReorderableListScope(state, orientation = Orientation.Horizontal, index, isAnimating),
    RowScope by scope
...
fun <T> DSReorderableColumn(
    ...
    onMove: () -> Unit = {},
    content: @Composable DSReorderableColumnScope.(index: Int, item: T, isDragging: Boolean) -> Unit,
) {
    val coroutineScope = rememberCoroutineScope()
    val density = LocalDensity.current
    val spacing = with(density) { verticalArrangement.spacing.toPx() }
    val layoutDirection = LocalLayoutDirection.current
    val DSReorderableListState = remember(list, spacing) {
        DSReorderableListState(
            list.size,
            spacing,
            onMove,
            onSettle,
    ...
    Column(
        ...
        list.forEachIndexed { i, item ->
            val isDragging by DSReorderableListState.isItemDragging(i)
            val isAnimating by DSReorderableListState.isItemAnimating(i)

            DSReorderableColumnScope(
                state = DSReorderableListState,
                index = i,
...
fun <T> DSReorderableRow(
    ...
    onMove: () -> Unit = {},
    content: @Composable DSReorderableRowScope.(index: Int, item: T, isDragging: Boolean) -> Unit,
) {
    val coroutineScope = rememberCoroutineScope()
    val density = LocalDensity.current
