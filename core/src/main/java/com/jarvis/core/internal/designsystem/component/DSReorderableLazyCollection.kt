...
import com.jarvis.core.internal.designsystem.utils.DSScroller
import com.jarvis.core.internal.designsystem.utils.DragGestureDetector
import com.jarvis.core.internal.designsystem.utils.draggable
import com.jarvis.core.internal.designsystem.utils.fromAxis
import com.jarvis.core.internal.designsystem.utils.getAxis
import com.jarvis.core.internal.designsystem.utils.minus
import com.jarvis.core.internal.designsystem.utils.opposite
import com.jarvis.core.internal.designsystem.utils.plus
import com.jarvis.core.internal.designsystem.utils.reverseAxis
...
object DSReorderableLazyCollectionDefaults {
    val ScrollThreshold = 48.dp
}

const val ScrollAmountMultiplier = 0.05f

data class AbsolutePixelPadding(
    val start: Float,
    val end: Float,
    val top: Float,
    val bottom: Float,
...
data class CollectionScrollPadding(
    val start: Float,
    val end: Float,
) {
    companion object {
...
@Stable
open class DSReorderableLazyCollectionState<out T> internal constructor(
    ...
    private val scroller: DSScroller,
    ...
    internal fun onDrag(offset: Offset) {
        ...
        val isScrollingStarted = if (distanceFromStart < scrollThreshold) {
            scroller.start(
                ...
                onScroll = {
                    moveDraggingItemToEnd(DSScroller.Direction.BACKWARD)
                }
            )
        } else if (distanceFromEnd < scrollThreshold) {
            scroller.start(
                DSScroller.Direction.FORWARD,
                ...
                onScroll = {
                    moveDraggingItemToEnd(DSScroller.Direction.FORWARD)
                }
            )
        } else {
            scroller.tryStop()
    ...
    private suspend fun moveDraggingItemToEnd(
        direction: DSScroller.Direction,
    ) {
        ...
        val isDraggingItemAtEnd = when (direction) {
            DSScroller.Direction.FORWARD -> draggingItem.index == state.layoutInfo.visibleItemsInfo.lastOrNull()?.index
            DSScroller.Direction.BACKWARD -> draggingItem.index == state.firstVisibleItemIndex
        }
        ...
        val targetItem = findTargetItem(
            ...
            when (direction) {
                DSScroller.Direction.FORWARD -> it.findLast(targetItemFunc)
                DSScroller.Direction.BACKWARD -> it.find(targetItemFunc)
            }
        }
        if (targetItem == null) {
            onMoveStateMutex.unlock()
        ...
        val isTargetDirectionCorrect = when (direction) {
            DSScroller.Direction.FORWARD -> targetItem.index > draggingItem.index
            DSScroller.Direction.BACKWARD -> targetItem.index < draggingItem.index
        }
    ...
    private fun findTargetItem(
        ...
        direction: DSScroller.Direction = DSScroller.Direction.FORWARD,
        ...
        val targetItem = when (direction) {
            DSScroller.Direction.FORWARD -> items.find(targetItemFunc)
            DSScroller.Direction.BACKWARD -> items.findLast(targetItemFunc)
        }
        return targetItem
    }
...
@Stable
interface DSReorderableCollectionItemScope {
    /**
     * Make the UI element the draggable handle for the reorderable item.
     *
     * This modifier can only be used on the UI element that is a child of [DSReorderableItem].
     *
...
internal class DSReorderableCollectionItemScopeImpl(
    private val DSReorderableLazyCollectionState: DSReorderableLazyCollectionState<*>,
    private val key: Any,
    private val itemPositionProvider: () -> Offset,
) : DSReorderableCollectionItemScope {
    /**
     * Make the UI element the draggable handle for the reorderable item.
     *
     * @param enabled Whether or not drag is enabled
    ...
    override fun Modifier.draggableHandle(
        ...
        onGloballyPositioned {
            ...
            key1 = DSReorderableLazyCollectionState,
            enabled = enabled && (DSReorderableLazyCollectionState.isItemDragging(key).value || !DSReorderableLazyCollectionState.isAnyItemDragging),
            ...
            onDragStarted = {
                coroutineScope.launch {
                    ...
                    DSReorderableLazyCollectionState.onDragStart(key, handleCenter)
                }
                onDragStarted(it)
            },
            onDragStopped = {
                DSReorderableLazyCollectionState.onDragStop()
                onDragStopped()
            },
            onDrag = { change, dragAmount ->
                change.consume()
                DSReorderableLazyCollectionState.onDrag(dragAmount)
            }
        )
    }
 ...
 */
@Composable
fun DSReorderableCollectionItem(
    state: DSReorderableLazyCollectionState<*>,
    key: Any,
    modifier: Modifier = Modifier,
    enabled: Boolean = true,
    dragging: Boolean,
    content: @Composable DSReorderableCollectionItemScope.(isDragging: Boolean) -> Unit,
) {
    ...
    Box(
        ...
        val itemScope = remember(state, key) {
            DSReorderableCollectionItemScopeImpl(
                DSReorderableLazyCollectionState = state,
                key = key,
